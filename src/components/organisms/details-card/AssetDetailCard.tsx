...
import { AssetDetailData, BaseAsset, ItemOwnerBalance } from "@/types";
...
export const AssetDetailCard = ({
  ...
  const [currentBeneficiary, setCurrentBeneficiary] = useState<AssetDetailData>(
    beneficiaries[0]
  );
  const assets = currentBeneficiary?.fwDetailAsset?.map((item) => {
    const asset = (ownerBalance ?? [])?.find(
      (owner) => owner.address === item?.asset
    );
    return {
      name: asset?.name,
      symbol: asset?.symbol,
  ...
  return (
    <CartItemContainer title="Asset details" iconTitle={<DiamondIcon />}>
      <Flex vertical gap={16}>
        <Text className="neutral-1">
          You’re a designated beneficiary of the following assets:
        </Text>
        <Beneficiaries
