...
export const AssetToBeneficiary = () => {
  ...
  const [currentSelected, setCurrentSelected] = useState<BeneficiaryData | null>(null);
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <IconButton onClick={() => {
          setAssets((pre) => pre.filter(asset => asset?.value !== record?.value));
  ...
  const handleAddConfigAsset = (asset: AssetSelectType, percent: number) => {
    ...
    const checkExist = assets?.findIndex((item) => item?.value === asset?.value);
    if (checkExist > -1) {
      const totalCurrentPercent = currentBeneficiaries?.reduce((acc: number, beneficiary: BeneficiaryConfig) => {
        if (beneficiary?.address === currentSelected?.address) {
          return acc + percent;
        }
        return acc + beneficiary?.assetConfig?.reduce((acc: number, item) => acc + (item?.asset?.value === asset?.value ? item?.percent : 0), 0);
      }, 0);
      if (totalCurrentPercent > 100) {
        WillToast.error(`Adding ${asset?.title} percentage exceeds the total allowed percentage of 100%`);
        return;
      }
      ...
      const totalCurrentPercent = currentBeneficiaries?.reduce((acc: number, item: BeneficiaryConfig) => {
        const totalPercent = item?.assetConfig?.reduce((acc: number, item) => acc + (item?.asset?.value === asset?.value ? item?.percent : 0), 0);
    ...
    const newBeneficiaries = currentBeneficiaries.map((beneficiary: BeneficiaryConfig) => {
      if (beneficiary?.address === currentSelected?.address) {
        const checkExist = beneficiary?.assetConfig?.findIndex((item) => item?.asset?.value === asset?.value);
        return {
          ...beneficiary,
          assetConfig: (checkExist > -1) ? (beneficiary?.assetConfig || [])?.map((item) => {
  ...
  const changeCurrentSelected = (beneficiary: BeneficiaryData) => {
    ...
    const currentBeneficiary = beneficiaries.find((item: BeneficiaryConfig) => item?.address === beneficiary?.address);
